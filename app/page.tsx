...
export default function HomePage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Expertise cards — scale up on scroll
    ...
    <div>
      ...
      <section ref={expertiseRef} className="border-t border-neutral-200 py-20 sm:py-28">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <ScrollReveal delay={0.2}>
            <p className="mb-12 max-w-xl text-neutral-500">
              Finance · Data · Digital — trois domaines complémentaires au service de solutions concrètes.
            </p>
          </ScrollReveal>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {AXES.map(({ icon: Icon, title, items, color, accent }) => (
              <div
                key={title}
                className={`expertise-card group relative overflow-hidden rounded-2xl border border-neutral-200 bg-gradient-to-br p-7 transition-all duration-500 hover:shadow-xl hover:shadow-neutral-900/5 hover:-translate-y-1 ${color}`}
              >
                <div className={`mb-5 inline-flex rounded-xl bg-white p-3 shadow-sm ${accent}`}>
      ...
      {featured.length > 0 && (
        <section ref={projectsRef} className="border-t border-neutral-200 py-20 sm:py-28">
          <div className="mx-auto max-w-5xl px-6">
            <div className="mb-10 flex items-end justify-between">
              <div>
                ...
                <p className="text-neutral-500">
                  Sélection de projets représentatifs de mon parcours.
                </p>
              </div>
              <Link
                href="/projects"
                className="mb-8 hidden items-center gap-1.5 text-sm font-medium text-neutral-600 hover:text-neutral-900 sm:inline-flex"
              >
                Tous les projets
                <ArrowRight size={14} />
              </Link>
            ...
            <Link
              href="/projects"
              className="mt-10 inline-flex items-center gap-1.5 text-sm font-medium text-neutral-600 hover:text-neutral-900 sm:hidden"
            >
      ...
      <section ref={ctaRef} className="border-t border-neutral-200 bg-neutral-900 py-20 sm:py-28">
        <div className="mx-auto max-w-5xl px-6 text-center">
          ...
          />
          <TextReveal
            text="Construisons quelque chose d'exceptionnel."
            as="p"
            delay={0.3}
          ...
          <Link
            href="/contact"
            className="group inline-flex items-center gap-2 rounded-full bg-white px-8 py-4 text-sm font-medium text-neutral-900 transition-all duration-300 hover:shadow-xl hover:shadow-white/10 hover:scale-105"
          >
            Me contacter
            <ArrowUpRight
              size={16}
